// Заказанная картина для проверки раскладки глазами: два плотных комьюнити должны
// разойтись по разные стороны, мостик — натянуться, одиночки — уйти на край.
...
const CLIQUE = 15; // знают друг друга все до одного
const FRINGE = 5; // знакомы примерно с половиной клики
const OTHER = 8;
const BRIDGE_LINKS = 2;

const INVITED_MAX = 5;
const INVITED_CROSS = 0.3; // шанс, что новичок знаком ещё с кем-то из первой компании

const CONNECTORS = 5;
const CONNECTOR_TO_OTHER = 3;
const CONNECTOR_TO_FIRST = 2;
const TAIL_PER_CONNECTOR = 5;
...
async function main(): Promise<void> {
	...
	// Пишем одной пачкой: полторы сотни отдельных вставок на слабом сервере медленны.
	...
	// Array.sort со случайным компаратором перемешивает неравномерно — Фишер—Йетс.
	...
	for (const host of first) {
		const count = Math.floor(random() * (INVITED_MAX + 1));
